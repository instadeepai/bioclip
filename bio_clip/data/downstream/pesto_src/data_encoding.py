...
def encode_features(structure, device=pt.device("cpu")):
    # charge features
    qe = pt.from_numpy(
        onehot(structure["element"], std_elements).astype(np.float32)
    ).to(device)
    qr = pt.from_numpy(
        onehot(structure["resname"], std_resnames).astype(np.float32)
    ).to(device)
    qn = pt.from_numpy(onehot(structure["name"], std_names).astype(np.float32)).to(
        device
    )
...
def extract_all_contacts(subunits, r_thr, device=pt.device("cpu")):
    ...
    for i in range(len(snames)):
        ...
        for j in range(i + 1, len(snames)):
            ...
            # find contacts
            ids_i, ids_j, d_ij = locate_contacts(
                subunits[cid_i]["xyz"], subunits[cid_j]["xyz"], r_thr, device=device
            )

            # insert contacts
            if (ids_i.shape[0] > 0) and (ids_j.shape[0] > 0):
                if f"{cid_i}" in contacts_dict:
                    contacts_dict[f"{cid_i}"].update(
                        {
                            f"{cid_j}": {
                                "ids": pt.stack([ids_i, ids_j], dim=1),
                                "d": d_ij,
                            }
                        }
                    )
                else:
                    contacts_dict[f"{cid_i}"] = {
                        f"{cid_j}": {"ids": pt.stack([ids_i, ids_j], dim=1), "d": d_ij}
                    }

                if f"{cid_j}" in contacts_dict:
                    contacts_dict[f"{cid_j}"].update(
                        {
                            f"{cid_i}": {
                                "ids": pt.stack([ids_j, ids_i], dim=1),
                                "d": d_ij,
                            }
                        }
                    )
                else:
                    contacts_dict[f"{cid_j}"] = {
                        f"{cid_i}": {"ids": pt.stack([ids_j, ids_i], dim=1), "d": d_ij}
                    }
