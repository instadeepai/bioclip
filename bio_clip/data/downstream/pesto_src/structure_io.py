...
def read_molecule_cif(filepath):
    ...
    if xyz.shape[0] == 0:
        mol = {
            "xyz": np.zeros((1, 3)),
            "element": np.array(
                [doc[0].find_value("_chem_comp_atom.type_symbol").lower().title()]
            ),
        }
    else:
        mol = {
            ...
            "element": np.array(
                [x for x in doc[0].find_loop("_chem_comp_atom.type_symbol")]
            ),
        }
    ...
    with open(filepath, "w") as fs:
        for cn in subunits:
            ...
            for i in range(N):
                ...
                # format pdb line
                pdb_line = "{:<6s}{:>5d} {:<4s} {:>3s} {:1s}{:>4d}    {:8.3f}{:8.3f}{:8.3f}{:6.2f}{:6.2f}          {:<2s}  ".format(  # noqa: E501
        ...
        for k in range(num_frames):
            ...
            for cn in subunits:
                ...
                for i in range(N):
                    ...
                    # format pdb line
                    pdb_line = "{:<6s}{:>5d} {:<4s} {:>3s} {:1s}{:>4d}    {:8.3f}{:8.3f}{:8.3f}{:6.2f}{:6.2f}          {:<2s}  ".format(  # noqa: E501
                        h, i + 1, n, rn, cn, ri, xyz[0], xyz[1], xyz[2], 0.0, bf, e
                    )
