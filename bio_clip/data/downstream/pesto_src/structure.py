...
def clean_structure(structure):
    ...
    ids_chains = np.where(
        np.array(chains).reshape(-1, 1) == np.unique(chains).reshape(1, -1)
    )[1]
...
def concatenate_chains(chains):
    ...
    # concatenate chains
    structure = {
        key: np.concatenate([chains[cid][key] for cid in chains]) for key in keys
    }

    # add chain information
    structure["chain_name"] = np.concatenate(
        [np.array([cid] * chains[cid]["xyz"].shape[0]) for cid in chains]
    )
...
def tag_hetatm_chains(structure):
    ...
    delta_hetatm = np.cumsum(
        np.abs(np.sign(np.concatenate([[0], np.diff(resids_hetatm)])))
    )

    # update chain name
    cids_hetatm = np.array(
        [
            f"{cid}:{hid}"
            for cid, hid in zip(structure["chain_name"][m_hetatm], delta_hetatm)
        ]
    )
