# taken from PeSTo/interfaceome/src/scoring.py
import numpy as np
import torch as pt
...
def roc_auc(y, p, P, N):
    ...
    if pt.any(m):
        a = np.array(
            roc_auc_score(y[:, m].cpu().numpy(), p[:, m].cpu().numpy(), average=None)
        )
...
def reg_scoring(y, p):
    return {
        ...
        "pcc": pearsonr(y.cpu().numpy(), p.cpu().numpy())[0]
        if not pt.allclose(y, y[0])
        else np.nan,
